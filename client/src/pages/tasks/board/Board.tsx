...
export default function Board() {
  ...
  // useRef to get the width of the board and then hard code it in to be variable with styed components.
  // * ghfgmhiodfhjdfg;gfd;oihfdh;oijdfg;hoifgdjxh;likgfdxjh;ilkj////////////////////////////////////////////////////////////

  const boardColumnsContainerRef = useRef<HTMLDivElement>(null)
  const boardColumnsContainerResizeObserver = useRef()

  const [boardColumnsContainerWidth, setBoardColumnsContainerWidth] = useState(0)


  
  const resizeBoardColumnsContainerHandler = (e: any) => {
    boardColumnsContainerRef.current && console.log(boardColumnsContainerRef.current.clientWidth);
  }

  useEffect(() => {

    boardColumnsContainerRef.current && new ResizeObserver(resizeBoardColumnsContainerHandler).observe(boardColumnsContainerRef.current);


    // if (boardColumnsContainerRef.current != null) {
    //   boardColumnsContainerResizeObserver.current = new ResizeObserver(resizeBoardColumnsContainerHandler).observe(boardColumnsContainerRef.current)
    // };

    // return () => {
    //   boardColumnsContainerResizeObserver.current.unobserve(boardColumnsContainerRef.current)
    // }
  ...
  return (
    <div className='board-container'>
      ...
      <section className='board-columns-container' ref={boardColumnsContainerRef}>
        {/* <div className="left-board-cover"></div> */}

        <div className="board-columns-content">
          <div className="board-columns">
            <div className='board-column backlog-column'><span>Backlog</span></div>
            <div className='board-column todo-column'><span>Todo</span></div>
            <div className='board-column in-progress-column'><span>In Progress</span></div>
            <div className='board-column in-review-column'><span>In Review</span></div>
            <div className='board-column done-column'><span>Completed</span></div>
            <div className='board-column cancelled-column'><span>Cancelled</span></div>
          </div>
        </div>

        {/* <div className="right-board-cover"></div> */}
      </section>
